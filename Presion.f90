...
!en las caras
   ...
   contains
      ...
      USE LeerData
  	 ...
  	 ALLOCATE(F2%value(ni-1,nj-1,nk-1))
   	 ALLOCATE(F3%value(ni-1,nj-1,nk-1))
         ...
         pp%title  = "Corrección de Presión"
          p%title  = "Presion"
         dpx%title = "Caída de Presión en X"
         dpy%title = "Caída de Presión en Y"
         dpz%title = "Caída de Presión en Z"
      ...
      SUBROUTINE calP(u,v,w)
  	 ...
  	 CALL initCoef()
   	 DO indiceDir = 1, 3
   	 !Coeficientes para la ecuación de corrección de presión P´
   	    DO i = 2,objMalla%l2
   	    DO j = 2,objMalla%m2
   	    DO k = 2,objMalla%n2
            ...
            SELECT CASE (indiceDir)
               ...
               CASE (1)
                  ...
                  IF (ni.gt.3.and.i.LE.objMalla%l2-1) THEN
                     ...
                     !Determinación de la densidad en la cara e
                     rhoFace = InterLin(rho%value(i,j,k),rho%value(i+1,j,k),i,1)

                     !Determinación de los gradientes de presión interpolados en la cara e
                     dpxel = InterLin(dpx%value(i,j,k),dpx%value(i+1,j,k),i,1)

                     !Determinación del gradiente de presión en la cara de la celda
                     dpxe  = (p%value(i+1,j,k) - p%value(i,j,k))/(objMalla%x(i+1) - objMalla%x(i))

                     !Interpolación de 1/ap en la cara e de la celda
                     ...
                     IF (visc%value(i,j,k).EQ.visc%value(i+1,j,k).AND.visc%value(i,j,k).LT.1.0D+25) THEN
               	        !Determinación de la velocidad u* en la cara de la celda e
               		...
               		!Corrección de la velocidad en la cara de la celda e
               		uface = upro - vol * dpf * (dpxe - dpxel)
               	     else
               	     	uface = 0.0D0
               	     END if

               	     !Corrección del flujo másico
               ...
               CASE (2)
                  ...
                  IF (nj.gt.3.and.j.LE.objMalla%m2-1) THEN
                     ...
                     IF (visc%value(i,j,k).EQ.visc%value(i,j+1,k).AND.visc%value(i,j,k).LT.1.0D+25) THEN
                        !Determinación de la velocidad v* en la cara de la celda n
                     	upro  = InterLin(v%value(i,j,k),v%value(i,j+1,k),j,2)

               		!Volumen alrededor de la cara
               		vol   = objMalla%areaY(i,k) * (objMalla%y(j+1) - objMalla%y(j))

               		!Corrección de la velocidad en la cara de la celda n
               		uface = upro - vol * dpf * (dpxe - dpxel)
               	     else
               	     	uface = 0.0D0
               	     END if
               ...
               CASE (3)
                  IF (nk.gt.3.and.k.LE.objMalla%n2-1) THEN
                     ...
                     IF (visc%value(i,j,k).EQ.visc%value(i,j,k+1).AND.visc%value(i,j,k).LT.1.0D+25) THEN
               		!Determinación de la velocidad w* en la cara de la celda t
               		upro  = InterLin(w%value(i,j,k),w%value(i,j,k+1),k,3)

               		!Volumen alrededor de la cara
               		vol   = objMalla%areaZ(i,j) * (objMalla%z(k+1) - objMalla%z(k))

               		!Corrección de la velocidad en la cara de la celda t
               		uface = upro - vol * dpf * (dpxe - dpxel)
               	     else
               	     	uface = 0.0D0
                     END IF
                     ...
                     !Cálculo de los coeficientes de p'
         ...
         DO k=2,objMalla%n2
            ...
            !Determina el desbalance de masa máximo
	 ...
	 END DO
  	 ...
  	 CALL solveEq(pp)
         ...
         CALL ppbound()
      	 ...
      	 ! Corrección de velocidades en los nodos y de los flujos
   	 DO i = 2,objMalla%l2
   	 DO j = 2,objMalla%m2
   	 DO k = 2,objMalla%n2
	 ...
	 END DO
      ...
      SUBROUTINE PDrop()
   	 ...
   	 ! Cálcula la caída de presión incluida en el término fuente
