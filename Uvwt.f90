...
!para las velocidades y la temperatura.
   ...
   CONTAINS
      ...
      !Dimensiona dinámicamente la velocidad y la temperatura (en memoria)
	 ...
	 END DO
      ...
      SUBROUTINE calUVW(timer,DT)
  	 ...
  	 FLX%PN1 = 0.0D0
   	 ...
   	 ! Cálculo de los coeficientes para la ecuación de cantidad de movimiento
   	 ! Cantidad de variables a resolver ==> velocidades U, V, y W

	 ! Construcción del lazo para todas las componentes de velocidad
         DO nf = 1,3
   	    SELECT CASE (nf)
      	    CASE (1) ! Velocidad U
    	       ...
    	       CALL initCoefBordes()
               ! Condiciones de contorno que varian por iteración
      ...
      SUBROUTINE calT(DT,timer)
  	 ...
  	 USE presion
   	 ...
   	 ! Cálculo de la Temperatura (ecuación de energía)
   	 nf = 5
   	 ! Inicializa todos los coeficientes a cero
	 CALL initCoef()
   	 ! Condiciones de contorno que varian por iteración
	 ...
	 DEALLOCATE(FLX%CK1,FLX%PK1)
      ...
      SUBROUTINE getCoefDominio()
   	 ...
   	 DO k = 2,objMalla%n2
         ...
         ! Cálculo de los coeficentes de dominio de los nodos vecinos en la dirección X,
         ! excluyéndo los bordes
          ...
          ! Cálculo de los coeficientes de dominio de los nodos vecinos en la dirección Y
          ! excluyéndo los bordes.
           ...
           ! Cálculo de los coeficientes de dominio de los nodos vecinos en la dirección Z
           ! excluyéndo los bordes.
      ...
      SUBROUTINE getCoefBounds(f)
         ...
         DO k = 2,objMalla%n2
            ...
            !Càlculo de coeficientes de los nodos vecinos en en borde izquierdo
            ! Borde I = 1
...
!	   ! Plano XZ
		 ...
		 flowIn   =  flowIn + objMalla%areaY(i,k) * rho%value(i,j,k) * v%value(i,j-1,k)
	      ...
	      k = 2             ! Cara B
           ...
           ! La corrección es realizada si la condición de contorno es igual a OUTPUT
