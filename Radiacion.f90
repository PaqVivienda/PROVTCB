...
MODULE adapt
   ...
   !VARIABLES QUE SE USAN PARA LA RADIACIÓN
   ...
   !Variables usadas en el módulo de radiación
   ...
   contains
      ...
      SUBROUTINE begin(u,v,w,t,TIMER)
         ...
         OPEN (UNIT = 11, FILE = nomRad, STATUS ='OLD')
            ...
            !Lectura de las propiedades nodo por nodo, desde el archivo de radiación
            DO a=1,objmalla%l1
            DO b=1,objmalla%m1
            DO c=1,objmalla%n1
   	 ...
   	 END IF
	       ...
	       !C/////// FIN CALCULO DE IRRADIANCIA TECHO Y PAREDES   /////
            ...
            DO i=2,objMalla%l2
               KBC.J1(i,k)=1
               KBC.M1(i,k)=1
               !c..... condición de flujo conocido y calculado"2" o  adiabatico"1"
               IF(ICBTE.NE.0)THEN
                  KBC.M1(i,k)=2
                  IF(ICBTE.EQ.2)THEN
         ...
         !c.....cambio de velocidad por flotación.......
         IF(NF.EQ.2)THEN
            DO k=2,objMalla%n2
            DO j=2,objMalla%m2
            DO i=2,objMalla%l2
